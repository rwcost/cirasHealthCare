...
-- Dumping data for table cirashea_myphr.user: 4 rows
...
INSERT INTO `user` (`userid`, `username`, `userpassword`, `userfname`, `userlname`) VALUES
	...
	(2402, 'GabMaster', 'keyword', 'Gab', 'Master'),
	(1023, 'Joe', NULL, NULL, NULL);
...
CREATE TABLE IF NOT EXISTS `user_vitals` (
  ...
  PRIMARY KEY (`key`)
) ENGINE=MyISAM AUTO_INCREMENT=203 DEFAULT CHARSET=utf8 COLLATE=utf8_bin COMMENT='table for patient vitals';

-- Dumping data for table cirashea_myphr.user_vitals: 96 rows
...
INSERT INTO `user_vitals` (`key`, `userid`, `date_time`, `systolic`, `diastolic`, `hdl`, `ldl`, `RAS`, `isOnMeds`, `isSmoker`, `age`, `isDiabetic`, `isMale`) VALUES
	...
	(21, 2042, '2012-12-10 18:38:33', 112, 64, 52, 124, 1, 0, 0, 23, 0, 0),
	(20, 2042, '2012-12-20 18:38:33', 125, 82, 52, 124, 1, 0, 0, 23, 0, 1),
	...
	(9, 1023, '2012-09-13 14:32:09', 145, 100, 31, 201, 25, 0, 1, 72, 0, 1),
	...
	(7, 1023, '2012-09-20 10:12:56', 160, 100, 31, 209, 29, 0, 1, 72, 0, 1),
	(8, 1023, '2012-09-27 10:11:56', 152, 100, 31, 190, 22, 0, 1, 72, 0, 1);
...
CREATE ALGORITHM=UNDEFINED DEFINER=`cirashea_rwcost`@`174.102.42.96` SQL SECURITY DEFINER VIEW `usermaxima` AS select `user_vitals`.`userid` AS `userid`,max(`user_vitals`.`RAS`) AS `max(RAS)`,max(`user_vitals`.`ldl`) AS `max(ldl)`,max(`user_vitals`.`hdl`) AS `max(hdl)`,max(`user_vitals`.`systolic`) AS `max(systolic)`,max(`user_vitals`.`diastolic`) AS `max(diastolic)` from `user_vitals` group by `user_vitals`.`userid`;


-- Dumping structure for view cirashea_myphr.usermeanima
-- Removing temporary table and create final VIEW structure
DROP TABLE IF EXISTS `usermeanima`;
CREATE ALGORITHM=UNDEFINED DEFINER=`cirashea_rwcost`@`174.102.42.96` SQL SECURITY DEFINER VIEW `usermeanima` AS select `user_vitals`.`userid` AS `userid`,avg(`user_vitals`.`RAS`) AS `avg(RAS)`,avg(`user_vitals`.`ldl`) AS `avg(ldl)`,max(`user_vitals`.`hdl`) AS `avg(hdl)`,max(`user_vitals`.`systolic`) AS `avg(systolic)`,avg(`user_vitals`.`diastolic`) AS `avg(diastolic)` from `user_vitals` group by `user_vitals`.`userid`;


-- Dumping structure for view cirashea_myphr.userminima
-- Removing temporary table and create final VIEW structure
DROP TABLE IF EXISTS `userminima`;
CREATE ALGORITHM=UNDEFINED DEFINER=`cirashea_rwcost`@`174.102.42.96` SQL SECURITY DEFINER VIEW `userminima` AS select `user_vitals`.`userid` AS `userid`,min(`user_vitals`.`RAS`) AS `min(RAS)`,min(`user_vitals`.`ldl`) AS `min(ldl)`,max(`user_vitals`.`hdl`) AS `min(hdl)`,max(`user_vitals`.`systolic`) AS `min(systolic)`,min(`user_vitals`.`diastolic`) AS `min(diastolic)` from `user_vitals` group by `user_vitals`.`userid`;
